<template>
	...
	>
		<img
			class="entrance touch-icon"
			src="./icon-entrance.png"
			v-show="!showSidebar"
			@click="toggleSidebar"
		/>
		<img
			class="close touch-icon"
			src="./icon-close.png"
			v-show="showSidebar"
			@click="toggleSidebar"
		/>
...
export default {
	...
	methods: {
		...
		moveEditor(x, y) {
			...
			const maxTop =
				window.innerHeight - (showSidebar ? this.height + 5 : 50);
			if (nextTop > maxTop) nextTop = maxTop;
			this.top = nextTop;
			let nextLeft = x;
			if (nextLeft < 0) nextLeft = 0;
			const maxLeft =
				window.innerWidth - (showSidebar ? this.width + 5 : 50);
...
<style>
.ext-panel .editor .el-tabs__content,
.ext-panel .editor .el-tabs--top,
.ext-panel .editor .el-tab-pane {
	height: 100%;
}
