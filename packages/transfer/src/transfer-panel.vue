		...
		filteredData() {
			return this.data.filter(item => {
				if (typeof this.filterMethod === "function") {
					...
					const label =
						item[this.labelProp] || item[this.keyProp].toString();
					return (
						label.toLowerCase().indexOf(this.query.toLowerCase()) >
						-1
					);
				}
			});
	...
	methods: {
		updateAllChecked() {
			...
			this.allChecked =
				...
				checkableDataKeys.every(
					item => this.checked.indexOf(item) > -1
				);
		},
