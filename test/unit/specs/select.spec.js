...
describe("Select", () => {
      ...
      `,
				...
				data() {
					return {
						...
						loading: false,
						filterMethod:
							configs.filterMethod && configs.filterMethod(this),
						remote: configs.remote,
						remoteMethod:
							configs.remoteMethod && configs.remoteMethod(this),
	...
	it("disabled option", done => {
		...
		setTimeout(() => {
			const options = vm.$el.querySelectorAll(
	...
	it("disabled select", () => {
		...
		expect(
			vm.$el.querySelector(".el-input").classList.contains("is-disabled")
		).to.true;
	});
	...
	it("multiple select", done => {
		...
		setTimeout(() => {
			...
			setTimeout(() => {
				...
				setTimeout(() => {
					expect(
						vm.value.indexOf("选项2") > -1 &&
							vm.value.indexOf("选项4") > -1
					).to.true;
					const tagCloseIcons = vm.$el.querySelectorAll(
						".el-tag__close"
					);
	...
	it("should set placeholder to label of selected option when filterable is true and multiple is false", async () => {
      ...
      `,
		...
		expect(
			vm.$el.querySelector(".el-input__inner").placeholder
		).to.be.equal("test");
