...
describe("Table", () => {
	...
	describe("attributes", () => {
		...
		it("tableRowStyle[Function]", done => {
			...
			setTimeout(_ => {
				let child1 = vm.$el.querySelector(
					".el-table__body tr:nth-child(1)"
				);
				let child2 = vm.$el.querySelector(
      ...
      `,
			...
			setTimeout(_ => {
				...
				setTimeout(_ => {
					...
					setTimeout(_ => {
						expect(tr.classList.contains("current-row")).to.be
							.false;
						expect(rows[1].classList.contains("current-row")).to.be
							.true;
						destroyVM(vm);
						done();
					}, DELAY);
	...
	describe("filter", () => {
		...
		it("render", () => {
			expect(vm.$el.querySelector(".el-table__column-filter-trigger")).to
				.exist;
		});

		it("click dropdown", done => {
			const btn = vm.$el.querySelector(
	...
	describe("events", () => {
		...
		it("cell-mouse-enter", done => {
			...
			setTimeout(_ => {
				const cell = vm.$el.querySelectorAll(
					".el-table__body .cell"
				)[2]; // first row
		...
		it("cell-mouse-leave", done => {
			...
			setTimeout(_ => {
				...
				const cell2 = vm.$el.querySelectorAll(
					".el-table__body .cell"
				)[2]; // first row
	...
	describe("column attributes", () => {
		...
		it("fixed", done => {
			...
			setTimeout(_ => {
				expect(
					toArray(
						vm.$el.querySelectorAll(
							".el-table__fixed th:not(.is-hidden)"
						)
					).map(node => node.textContent)
				).to.eql(["test1", "test3"]);
				...
				expect(
					toArray(
						vm.$el.querySelectorAll(
							".el-table__fixed-right th:not(.is-hidden)"
						)
					).map(node => node.textContent)
				).to.eql(["test2"]);
				expect(
		...
		it("class-name", done => {
			...
			setTimeout(_ => {
				...
				expect(vm.$el.querySelectorAll(".column-class-a")).to.length(
					len * 2
				);
				destroyVM(vm);
				done();
        ...
        `,
			...
			setTimeout(_ => {
				expect(vm.$el.querySelector(".el-checkbox").__vue__.checked).to
					.be.false;
				setTimeout(_ => {
					expect(vm.selected).to.length(0);
					destroyVM(vm);
		...
		describe("type", () => {
          ...
          `,
			...
			describe("= selection", () => {
				...
				it("render", done => {
					setTimeout(_ => {
						expect(
							vm.$el.querySelectorAll(".el-checkbox")
						).to.length(getTestData().length + 1);
						done();
					}, DELAY);
				});
				...
				it("select one", done => {
					...
					setTimeout(_ => {
						...
						setTimeout(_ => {
							...
							expect(vm2.selected[0].name).to.equal(
								getTestData()[0].name
							);
							destroyVM(vm2);
							done();
          ...
          `,
				...
				it("should default-expand-all when default-expand-all is true", done => {
					...
					setTimeout(_ => {
						expect(
							vm.$el.querySelectorAll(".el-table__expanded-cell")
								.length
						).to.equal(5);
						destroyVM(vm);
						done();
		...
		describe("sortable", () => {
			it("render", done => {
				...
				setTimeout(_ => {
					expect(vm.$el.querySelectorAll(".caret-wrapper")).to.length(
						1
					);
					destroyVM(vm);
					done();
	...
	describe("multi level column", () => {
        ...
        `,
			...
			setTimeout(_ => {
				...
				const firstRowHeader = trs[0].querySelectorAll("th .cell")
					.length;
				const secondRowHeader = trs[1].querySelectorAll("th .cell")
					.length;
        ...
        `,
			...
			setTimeout(_ => {
				...
				const secondRowHeader = trs[1].querySelectorAll("th .cell")
					.length;
				const thirdRowHeader = trs[2].querySelectorAll("th .cell")
					.length;
				expect(firstRowHeader).to.equal(3);
				expect(secondRowHeader).to.equal(2);
        ...
        `,
			...
			setTimeout(_ => {
				const trs = vm.$el.querySelectorAll(".el-table__header tr");
				expect(trs.length).equal(2);
				const firstRowLength = trs[0].querySelectorAll("th .cell")
					.length;
				const secondRowLength = trs[1].querySelectorAll("th .cell")
					.length;
	...
	describe("dynamic column attribtes", () => {
        ...
        `,
			...
			setTimeout(() => {
				expect(
					vm.$el.querySelector(".el-table__header th .cell")
						.textContent
				).to.equal("name");
				vm.label = "NAME";
				vm.$nextTick(() => {
        ...
        `,
			...
			setTimeout(() => {
				expect(
					vm.$el.querySelectorAll(".el-table__body td.is-right")
						.length === 0
				).to.be.true;
				vm.align = "right";
				vm.$nextTick(() => {
        ...
        `,
			...
			vm.$nextTick(() => {
				expect(
					vm.$el.querySelectorAll(".el-table__header th.is-left")
						.length
				).to.above(0);
				expect(
					vm.$el.querySelectorAll(".el-table__header th.is-center")
						.length
				).to.equal(0);
				expect(
					vm.$el.querySelectorAll(".el-table__header th.is-right")
						.length
				).to.equal(0);
				vm.align = "right";
				vm.$nextTick(() => {
        ...
        `,
			...
			setTimeout(() => {
				expect(
					vm.$el
						.querySelector(".el-table__body col")
						.getAttribute("width")
				).to.equal("100");
				vm.width = 200;
				setTimeout(() => {
        ...
        `,
			...
			setTimeout(() => {
				...
				vm.fixed = true;
				setTimeout(() => {
					expect(!!vm.$el.querySelector(".el-table__fixed")).to.be
						.true;
					destroyVM(vm);
					done();
				}, 100);
        ...
        `,
			...
			setTimeout(() => {
				...
				vm.prop = "release";
				setTimeout(() => {
					firstColumnContent = vm.$el.querySelector(
						".el-table__body td .cell"
					).textContent;
					secondColumnContent = vm.$el.querySelector(
						".el-table__body td:nth-child(2) .cell"
					).textContent;
					expect(firstColumnContent === secondColumnContent).to.be
						.true;
					destroyVM(vm);
					done();
				}, 100);
			...
			setTimeout(_ => {
				...
				setTimeout(_ => {
					...
					setTimeout(_ => {
						...
						expect(rows[1].classList.contains("current-row")).to.be
							.false;
						expect(rows[3].classList.contains("current-row")).to.be
							.true;
						destroyVM(vm);
						done();
					}, DELAY);
	...
	it("keep highlight row when data change", done => {
      ...
      `,
		...
		setTimeout(() => {
			let rows = vm.$el.querySelectorAll(
				".el-table__body-wrapper tbody tr"
			);
	...
	describe("tree", () => {
        ...
        `,
			...
			const childRows = vm.$el.querySelectorAll(
        ...
        `,
			...
			expect(
				expandIcon.classList.contains("el-table__expand-icon--expanded")
			).to.be.true;
			expect(vm.$el.querySelectorAll(".el-table__row").length).to.equal(
				8
			);
		});
        ...
        `,
					...
					methods: {
						...
						closeExpandRow() {
							...
							const row =
								testData[testData.length - 1].children[0];
			...
			const expandIcon = childRows[0].querySelector(
