...
describe("DatePicker", () => {
	...
	it("change event", done => {
		...
		setTimeout(_ => {
			...
			setTimeout(_ => {
				...
				setTimeout(_ => {
					expect(onChangeValue.getTime()).to.equal(
						vm.value.getTime()
					);
					done();
				}, DELAY);
		...
		it("works for type=daterange", done => {
          ...
          <el-date-picker
			...
			setTimeout(_ => {
				...
				setTimeout(_ => {
					picker.$el
						.querySelector("td.available + td.available")
						.click();
					setTimeout(_ => {
						expect(emitted).to.true;
						emitted = false;
	...
	describe("value-format", () => {
		...
		it("works for daterange, is timestamp", done => {
          ...
          <el-date-picker
			...
			setTimeout(_ => {
				// check timestamp is parsed internally
				expect(vm.$refs.compo.parsedValue[0].getTime()).to.equal(
					vm.value[0]
				);
				expect(vm.$refs.compo.parsedValue[1].getTime()).to.equal(
					vm.value[1]
				);
				inputs[0].value = "2000-10-01";
				triggerEvent(inputs[0], "input");
	...
	describe("default value", () => {
		...
		it("it works", done => {
			...
			setTimeout(_ => {
				const $el = vm.$refs.compo.picker.$el;
				expect($el.querySelector(".el-month-table td.default")).to
					.exist;
				expect($el.querySelector(".el-year-table td.default")).to.exist;
				const defaultEls = $el.querySelectorAll(
				...
				setTimeout(_ => {
					...
					setTimeout(_ => {
						...
						setTimeout(() => {
							...
							expect(
								$el.querySelector(".el-date-table td.current")
							).to.not.exist;
							expect(
								$el.querySelector(".el-date-table td.default")
							).to.exist;
							$el.querySelector(
								".el-date-table td.default"
							).click();
		...
		it("arrow keys navigates", done => {
			...
			vm.$nextTick(_ => {
				...
				setTimeout(_ => {
					...
					expect(datePicker.picker.date.getDate()).to.equal(
						prevDate.getDate()
					);
					done();
				}, DELAY);
	...
	describe("navigation", () => {
		...
		const initNavigationTest = async value => {
			...
			getYearLabel = () =>
				$el.querySelectorAll(".el-date-picker__header-label")[0]
					.textContent;
			getMonthLabel = () =>
				$el.querySelectorAll(".el-date-picker__header-label")[1]
					.textContent;
		};
	...
	it("type:month", done => {
		...
		setTimeout(_ => {
			expect(vm.picker.$el.querySelector(".el-month-table").style.display)
				.to.be.empty;
			expect(
	...
	it("type:year", done => {
		...
		setTimeout(_ => {
			expect(vm.picker.$el.querySelector(".el-year-table").style.display)
				.to.empty;
			expect(
	...
	describe("type:datetime", () => {
		...
		it("create", done => {
			...
			setTimeout(_ => {
				expect(
					vm.$refs.compo.picker.$el.querySelector(".el-time-panel")
				).to.ok;
				done();
			}, DELAY);
		});
		...
		it("click now button", done => {
			...
			setTimeout(_ => {
				...
				vm.picker.$el
					.querySelector(".el-picker-panel__link-btn")
					.click();
				setTimeout(_ => {
		...
		it("click timepicker", done => {
			...
			setTimeout(_ => {
				...
				setTimeout(_ => {
					const timePanel = vm.picker.$el.querySelector(
						".el-time-panel"
					);
					expect(window.getComputedStyle(timePanel)).to.not.equal(
						"none"
					);
					done();
				}, DELAY);
				...
				setTimeout(_ => {
					...
					setTimeout(_ => {
						...
						setTimeout(_ => {
							...
							cancel.click();
							setTimeout(_ => {
								expect(vm.value.toISOString()).to.equal(
									oldValue
								);
								expect(vm.$refs.compo.pickerVisible).to.true;
								done();
	...
	describe("type:week", () => {
		...
		it("click cell", done => {
			vm.picker.$el
				.querySelector(".el-date-table__row .available")
				.click();
			setTimeout(_ => {
		...
		it("click year label", () => {
			vm.picker.$el
				.querySelector(".el-date-picker__header-label")
				.click();
		...
		it("highlight correctly", done => {
			...
			setTimeout(() => {
				...
				const numberOfHighlightRows = () =>
					pickerEl.querySelectorAll(".el-date-table__row.current")
						.length;
				...
				setTimeout(() => {
					...
					setTimeout(() => {
						...
						setTimeout(() => {
							...
							pickerEl
								.querySelector(".el-icon-arrow-right")
								.click();
							setTimeout(() => {
								expect(numberOfHighlightRows()).to.equal(0);
								// test: next year should not have highlight
								pickerEl
									.querySelector(".el-icon-arrow-left")
									.click(); // go back one month
								pickerEl
									.querySelector(".el-icon-d-arrow-right")
									.click();
								setTimeout(() => {
									expect(numberOfHighlightRows()).to.equal(0);
									done();
				...
				setTimeout(_ => {
					...
					setTimeout(_ => {
						...
						setTimeout(_ => {
							...
							expect(
								vm.value[0].getTime() < vm.value[1].getTime()
							).to.be.true;
							done();
						}, DELAY);
					}, DELAY);
		...
		it("type:daterange unlink:true", done => {
			...
			setTimeout(_ => {
				...
				setTimeout(_ => {
					...
					const left = panels[0].querySelector(
		...
		it("unlink panels", done => {
			...
			setTimeout(_ => {
				...
				setTimeout(_ => {
					...
					expect(
						(rightText[1] <= 2 ? rightText[0] - 1 : rightText[0]) -
							leftText[0]
					).to.equal(1);
					expect(
						(rightText[1] <= 2 ? rightText[1] + 12 : rightText[1]) -
							leftText[1]
					).to.equal(2);
				...
				setTimeout(_ => {
					...
					setTimeout(_ => {
						...
						setTimeout(_ => {
							...
							input.focus();
							setTimeout(_ => {
								const startCell = picker.$el.querySelector(
									"td.start-date"
								);
								const endCell = picker.$el.querySelector(
									"td.end-date"
								);
								startCell.click();
								setTimeout(_ => {
				...
				setTimeout(_ => {
					...
					setTimeout(_ => {
						...
						setTimeout(_ => {
							triggerEvent(rightCell, "mousemove", true);
							setTimeout(_ => {
								expect(rightCell.classList.contains("in-range"))
									.to.be.true;
								...
								setTimeout(_ => {
									...
									expect(minDate.getMinutes()).to.be.equal(
										59
									);
									expect(minDate.getSeconds()).to.be.equal(
										59
									);
			...
			setTimeout(_ => {
				...
				setTimeout(_ => {
					...
					setTimeout(_ => {
						...
						triggerEvent(input, "change", true);
						setTimeout(_ => {
							expect(vm.picker.maxDate >= vm.picker.minDate).to
								.true;
							done();
						}, DELAY);
					}, DELAY);
					...
					data() {
						return {
							pickerOptions: {
								disabledDate(date) {
									return (
										date.getTime() < new Date(2000, 9, 1)
									); // 2000-10-01
								}
							},
							value: [
			...
			setTimeout(_ => {
				...
				setTimeout(_ => {
					...
					setTimeout(_ => {
						picker.$el
							.querySelector(
								"td:not(.disabled) ~ td:not(.disabled)"
							)
							.click();
						setTimeout(_ => {
							expect(spy.calledOnce).to.equal(true);
	...
	it("picker-options:shortcuts", done => {
		...
		setTimeout(_ => {
			const shortcut = vm.picker.$el.querySelector(
				".el-picker-panel__shortcut"
			);

			expect(shortcut.textContent).to.be.equal("今天");
			expect(vm.picker.$el.querySelector(".el-picker-panel__sidebar")).to
				.be.ok;
        ...
        `,
			...
			setTimeout(() => {
				...
				setTimeout(_ => {
					...
					expect(disabledHours[disabledHours.length - 2]).to.equal(
						16
					);
					expect(disabledHours[disabledHours.length - 1]).to.equal(
						23
					);
