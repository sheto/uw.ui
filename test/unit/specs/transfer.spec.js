...
describe("Transfer", () => {
	...
	it("filterable", done => {
		vm = createTransfer(
			'v-model="value" filterable :filter-method="method"',
			{
				data() {
					return {
						value: [],
		...
		const leftList = transfer.$el.querySelector(".el-transfer-panel")
			.__vue__;
		leftList.query = "1";
		setTimeout(_ => {
	...
	it("check", () => {
		...
		const leftList = vm.$refs.transfer.$el.querySelector(
			".el-transfer-panel"
		).__vue__;
	...
	describe("target order", () => {
		it("original(default)", done => {
			vm = createTransfer(
				'v-model="value" :left-default-checked="[2, 3]"',
				{
					data() {
						return {
							value: [1, 4]
			...
			setTimeout(() => {
				...
				setTimeout(() => {
					...
					expect(
						targetItems.map(item => item.innerText)
					).to.deep.equal([
						"备选项 1",
						"备选项 2",
						"备选项 3",
