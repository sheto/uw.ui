...
describe("Carousel", () => {
	...
	it("create", () => {
      ...
      `
		...
		expect(vm.$el.querySelectorAll(".el-carousel__item").length).to.equal(
			3
		);
	});
	...
	it("label", done => {
      ...
      `
		...
		setTimeout(_ => {
			expect(
				vm.$el.querySelector(".el-carousel__button").innerText
			).to.equal("1");
			done();
		}, 10);
	});
      ...
      `
			...
			setTimeout(() => {
				...
				setTimeout(() => {
					...
					setTimeout(() => {
						expect(items[0].classList.contains("is-active")).to
							.true;
						done();
					}, 10);
				}, 10);
		...
		setTimeout(() => {
			...
			setTimeout(() => {
				...
				setTimeout(() => {
					...
					items[6].click();
					setTimeout(() => {
						expect(items[6].classList.contains("is-active")).to
							.true;
						done();
					}, 10);
				}, 10);
	...
	it("vertical direction", () => {
      ...
      `
		...
		expect(items[0].style.transform.indexOf("translateY") !== -1).to.be
			.true;
	});
});
