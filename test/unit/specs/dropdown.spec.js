...
describe("Dropdown", () => {
	...
	it("split button", done => {
      ...
      `
		...
		let dropdown = vm.$refs.dropdown;
		let dropdownElm = dropdown.$el;
		let triggerElm = dropdownElm.querySelector(
	...
	it("updatePopper", done => {
      ...
      `
		...
		triggerEvent(triggerElm, "mouseenter");
		setTimeout(() => {
			const zIndex1 = document.querySelector(".el-dropdown-menu").style
				.zIndex;
			dropdown.broadcast("ElDropdownMenu", "updatePopper");
			setTimeout(() => {
				const zIndex2 = document.querySelector(".el-dropdown-menu")
					.style.zIndex;
