...
describe("Radio", () => {
	...
	describe("Radio group", () => {
        ...
        `,
			...
			setTimeout(_ => {
				expect(vm.$refs.radio1.$el.querySelector(".is-checked")).to.be
					.ok;
        ...
        `,
			...
			expect(
				vm.$el.querySelectorAll("label.is-disabled").length
			).to.be.equal(3);
			expect(vm.$refs.radio1.$el.querySelector(".is-checked")).to.be
				.exist;
			radio2.$el.click();
        ...
        `,
			...
			let radio2 = vm.$refs.radio2;
			expect(vm.$el.querySelectorAll(".is-disabled").length).to.be.equal(
				3
			);
			expect(vm.$refs.radio1.$el.classList.contains("is-active")).to.be
				.true;
			radio2.$el.click();
			setTimeout(_ => {
				expect(vm.radio === 3).to.be.true;
				expect(vm.$refs.radio1.$el.classList.contains("is-active")).to
					.be.true;
				done();
			}, 10);
		});
		...
		describe("Radio Button", () => {
          ...
          `,
				...
				radio.$el.click();
				setTimeout(_ => {
					expect(radio.$el.classList.contains("is-active")).to.be
						.true;
					expect(vm.radio === 6).to.be.true;
					done();
				}, 10);
          ...
          `,
				...
				setTimeout(_ => {
					expect(
						vm.$refs.radio1.activeStyle.backgroundColor
					).to.equal("#000");
					expect(vm.$refs.radio1.activeStyle.borderColor).to.equal(
						"#000"
					);
          ...
          `,
				...
				setTimeout(_ => {
					expect(
						vm.$el.querySelectorAll(".el-radio-button--large")
							.length
					).to.be.equal(3);
					done();
				}, 10);
