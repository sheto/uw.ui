...
describe("Menu", () => {
	...
	it("menu-item disabled", done => {
      ...
      `
		...
		setTimeout(_ => {
			expect(vm.$refs.item1.$el.classList.contains("is-active")).to.be
				.false;
			done();
		}, 20);
	});
	describe("default active", () => {
        ...
        `
			...
			expect(vm.$refs.item2.$el.classList.contains("is-active")).to.be
				.true;
			vm.$refs.item1.$el.click();
			setTimeout(_ => {
				expect(vm.$refs.item1.$el.classList.contains("is-active")).to.be
        ...
        `,
			...
			setTimeout(_ => {
				vm.active = "1";
				setTimeout(_ => {
					expect(vm.$refs.item1.$el.classList.contains("is-active"))
						.to.be.true;
					done();
				}, 20);
			}, 100);
        ...
        `
			...
			expect(vm.$refs.submenuItem2.$el.classList.contains("is-active")).to
				.be.true;

			setTimeout(_ => {
				expect(vm.$refs.submenu.$el.classList.contains("is-opened")).to
					.be.true;
				expect(vm.$refs.submenu.$el.classList.contains("is-active")).to
					.be.true;
				done();
			}, 20);
		});
	...
	describe("submenu", function() {
        ...
        `,
			...
			setTimeout(_ => {
				...
				setTimeout(_ => {
					expect(submenuItem2.$el.classList.contains("is-active")).to
						.be.true;
					submenu.$el.querySelector(".el-submenu__title").click();
					setTimeout(_ => {
						expect(submenu.$el.classList.contains("is-opened")).to
							.not.true;
						done();
					}, 20);
				}, 20);
        ...
        `,
			...
			expect(vm.$refs.submenu1.$el.classList.contains("is-opened")).to.be
				.true;
			expect(vm.$refs.submenu2.$el.classList.contains("is-opened")).to.be
				.true;
			vm.defaultOpeneds = ["2"];
			setTimeout(_ => {
				expect(vm.$refs.submenu1.$el.classList.contains("is-opened")).to
					.be.true;
				expect(vm.$refs.submenu2.$el.classList.contains("is-opened")).to
					.not.true;
				done();
			}, 20);
		});
	...
	it("unique-opened", done => {
      ...
      `,
		...
		setTimeout(_ => {
			expect(vm.$refs.submenu1.$el.classList.contains("is-opened")).to.not
				.true;
			done();
		}, 20);
	});
	it("horizontal mode", done => {
      ...
      `,
		...
		setTimeout(_ => {
			expect(
				document.body.querySelector(".el-menu--popup").parentElement
					.style.display
			).to.not.ok;
			done();
		}, 500);
	...
	it("menu group", () => {
      ...
      `
		...
		expect(
			vm.$refs.group1.$el.querySelector(".el-menu-item-group__title")
				.innerText
		).to.be.equal("分组一");
	});
