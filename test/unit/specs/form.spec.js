...
describe("Form", () => {
	...
	it("label width", done => {
      ...
      `,
		...
		expect(
			vm.$el.querySelector(".el-form-item__label").style.width
		).to.equal("80px");
		expect(
	...
	it("label position", done => {
      ...
      `,
		...
		expect(vm.$refs.labelTop.$el.classList.contains("el-form--label-top"))
			.to.be.true;
		expect(vm.$refs.labelLeft.$el.classList.contains("el-form--label-left"))
			.to.be.true;
		done();
	});
	it("label size", () => {
      ...
      `,
		...
		vm.$refs.form.validate(valid => {
			...
			vm.$refs.form.$nextTick(_ => {
				expect(vm.$el.querySelector(".el-form-item__error")).to.not
					.exist;
				done();
			});
		});
	...
	describe("validate", () => {
			...
			vm.$refs.form.validate(valid => {
				...
				setTimeout(_ => {
					...
					setTimeout(_ => {
						...
						const input = vm.$refs.picker.$el.querySelector(
							"input"
						);
						input.blur();
						input.focus();
        ...
        `,
			...
			vm.$nextTick(_ => {
				expect(vm.$refs.field.validateMessage).to.equal(
					"您需要接受用户协议"
				);
			});
			vm.$refs.form.validate(valid => {
