...
describe("Input", () => {
	...
	it("limit input and show word count", async () => {
      ...
      `,
		...
		expect(inputElm1.querySelectorAll(".el-input__count").length).to.equal(
			0
		);
		expect(inputElm2.querySelectorAll(".el-input__count").length).to.equal(
			1
		);
		expect(inputElm3.querySelectorAll(".el-input__count").length).to.equal(
			0
		);
