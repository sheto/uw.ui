...
describe("Calendar", () => {
	...
	it("create", async () => {
      ...
      `,
		...
		expect(vm.$el.querySelector(".is-selected span").innerText).to.be.equal(
			"6"
		);
	});
	...
	it("range", () => {
      ...
      `
		...
		expect(vm.$el.querySelector(".el-calendar__button-group")).to.be.a(
			"null"
		);
	});
	...
	it("range tow monthes", async () => {
      ...
      `
		...
		const titleEl = vm.$el.querySelector(".el-calendar__title");
		expect(/2019.*4/.test(titleEl.innerText)).to.be.true;
		const dateTables = vm.$el.querySelectorAll(
