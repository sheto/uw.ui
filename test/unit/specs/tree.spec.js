...
describe("Tree", () => {
	...
	it("emptyText", done => {
		...
		vm.$nextTick(() => {
			expect(
				vm.$el.querySelectorAll(".el-tree__empty-block").length
			).to.equal(1);
			done();
		});
	});
	...
	it("checkOnNodeClick", done => {
		vm = getTreeVm(
			':props="defaultProps" show-checkbox check-on-click-node'
		);
		const firstNode = document.querySelector(".el-tree-node");
		firstNode.click();
		vm.$nextTick(() => {
	...
	it("defaultCheckedKeys & check-strictly = false", () => {
		...
		expect(
			vm.$el.querySelectorAll(".el-checkbox .is-checked").length
		).to.equal(3);
	});
	...
	it("show checkbox", done => {
		...
		const tree = vm.$children[0];
		const secondNode = document.querySelectorAll(
	...
	it("updateKeyChildren", done => {
		...
		vm.$nextTick(() => {
			...
			expect(tree.store.nodesMap["1"].childNodes[0].data.id).to.equal(
				111
			);
