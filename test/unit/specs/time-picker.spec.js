...
describe("TimePicker", () => {
	...
	it("set AM/PM format", done => {
		...
		setTimeout(_ => {
			const list = vm.picker.$el.querySelectorAll(
				".el-time-spinner__list"
			);
			const hoursEl = list[0];
			expect(
				hoursEl.querySelectorAll(".el-time-spinner__item")[0]
					.textContent
			).to.equal("12 AM");
			expect(
				hoursEl.querySelectorAll(".el-time-spinner__item")[1]
					.textContent
			).to.equal("01 AM");
			expect(
				hoursEl.querySelectorAll(".el-time-spinner__item")[12]
					.textContent
			).to.equal("12 PM");
			expect(
				hoursEl.querySelectorAll(".el-time-spinner__item")[15]
					.textContent
			).to.equal("03 PM");
			done();
		}, DELAY);
	...
	it("select time", done => {
		...
		setTimeout(_ => {
			...
			const hourEl = hoursEl.querySelectorAll(
	...
	it("click cancel button", done => {
		...
		setTimeout(_ => {
			timePicker.picker.$el
				.querySelector(".el-time-panel__btn.cancel")
				.click();
	...
	it("click confirm button", done => {
		...
		setTimeout(_ => {
			timePicker.picker.$el
				.querySelector(".el-time-panel__btn.confirm")
				.click();
	...
	it("set format", done => {
		...
		setTimeout(_ => {
			expect(
				vm.picker.$el.querySelectorAll(".el-time-spinner__wrapper")[2]
					.style.display
			).to.equal("none");
			done();
		}, 20);
	...
	it("selectableRange", done => {
		...
		setTimeout(_ => {
			...
			expect(disabledHours).to.not.include.members([
				17,
				18,
				19,
				20,
			...
			setTimeout(_ => {
				const disabledMinutes = [].slice
					...
					.map(node => Number(node.textContent));
				expect(disabledMinutes.every(t => t > 30 && t < 50)).to.equal(
					true
				);
