	...
	it("select", done => {
      ...
      `,
		...
		setTimeout(_ => {
			...
			setTimeout(_ => {
				expect(inputElm.value).to.be.equal(
					"Hot honey 首尔炸鸡（仙霞路）"
				);
		...
		it("select", done => {
			...
			setTimeout(() => {
				...
				setTimeout(() => {
					...
					setTimeout(() => {
						expect(vm.item.address).to.be.equal(
							"长宁区新渔路144号"
						);
						done();
					}, 200);
      ...
      `,
		...
		setTimeout(_ => {
			...
			vm.$nextTick(_ => {
				...
				expect(highlightedItem.classList.contains("highlighted")).to.be
					.true;
				expect(suggestions.scrollTop === highlightedItem.scrollHeight)
					.to.be.true;
				done();
			});
		}, 500);
					...
					querySearch(queryString, cb) {
						...
						cb(
							queryString
								? opts.filter(
										opt =>
											opt.value.indexOf(queryString) >= 0
								  )
								: opts
						);
					}
