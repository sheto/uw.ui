...
describe("Tabs", () => {
	...
	it("dynamic", done => {
      ...
      `,
		...
		setTimeout(() => {
			...
			setTimeout(() => {
				expect(vm.$el.querySelectorAll(".el-tab-pane").length).to.equal(
					5
				);
				done();
			});
		...
		setTimeout(_ => {
			...
			setTimeout(_ => {
				...
				setTimeout(_ => {
					...
					expect(tabList[2].classList.contains("is-active")).to.be
						.true;
					done();
				}, 100);
			}, 100);
		...
		setTimeout(_ => {
			...
			setTimeout(_ => {
				...
				setTimeout(_ => {
					expect(tabList.length).to.be.equal(2);
					expect(paneList.length).to.be.equal(2);
					expect(tabList[1].classList.contains("is-active")).to.be
						.true;
					done();
				}, 100);
			}, 100);
	...
	it("closable in tab-pane", done => {
      ...
      `
		...
		setTimeout(() => {
			expect(vm.$el.querySelectorAll(".el-icon-close").length).to.equal(
				2
			);
			done();
		}, 100);
      ...
      `
		...
		setTimeout(_ => {
			...
			vm.$nextTick(() => {
				...
				vm.$nextTick(() => {
					expect(
						vm.$el.querySelector(".el-tabs__content").children
							.length
					).to.be.equal(4);
					expect(
						vm.$el.querySelector(".el-tabs__content > #pane-D")
