...
describe("Drawer", () => {
	...
	it("render correct content", async () => {
      ...
      `,
		...
		await waitImmediate();
		expect(
			vm.$el.querySelector(".el-drawer__body span").textContent
		).to.equal("这是一段信息");
		const footerBtns = vm.$el.querySelectorAll(".el-button");
		expect(footerBtns.length).to.equal(2);
		expect(footerBtns[0].querySelector("span").textContent).to.equal(
			"取消"
		);
		expect(footerBtns[1].querySelector("span").textContent).to.equal(
			"确定"
		);
	});
	...
	describe("size", () => {
      ...
      `,
		...
		it("should effect height when drawer is vertical", async () => {
			...
			expect(vm.$el.querySelector(".el-drawer").style.width).to.equal(
				"50%"
			);
		});
		...
		it("should effect width when drawer is horizontal", async () => {
			...
			expect(vm.$el.querySelector(".el-drawer").style.height).to.equal(
				"50%"
			);
		});
	});
