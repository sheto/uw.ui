...
describe("Progress", () => {
	...
	it("create", () => {
      ...
      `
		...
		expect(
			vm.$refs.percent0.$el.querySelector(".el-progress-bar__inner").style
				.width
		).to.be.equal("0%");
		...
		expect(
			vm.$refs.percent50.$el.querySelector(".el-progress-bar__inner")
				.style.width
		).to.be.equal("50%");

		expect(
			vm.$refs.percent100.$el.querySelector(".el-progress__text")
				.innerText
		).to.be.equal("100%");
		expect(
			vm.$refs.percent100.$el.querySelector(".el-progress-bar__inner")
				.style.width
		).to.be.equal("100%");
	});
	it("status", () => {
      ...
      `
		...
		expect(vm.$refs.lineSuccess.$el.classList.contains("is-success")).to.be
			.true;
		expect(
		...
		expect(vm.$refs.lineException.$el.classList.contains("is-exception")).to
			.be.true;
		expect(
		...
		expect(vm.$refs.circleSuccess.$el.classList.contains("is-success")).to
			.be.true;
		expect(
		...
		expect(vm.$refs.circleException.$el.classList.contains("is-exception"))
			.to.be.true;
		expect(
	...
	it("text inside", () => {
      ...
      `
		...
		expect(vm.$el.classList.contains("el-progress--text-inside")).to.be
			.true;
	});
	it("stroke width", () => {
		vm = createVue(
	...
	it("width", () => {
      ...
      `
		...
		expect(
			vm.$el.querySelector(".el-progress-circle").style.width
		).to.be.equal("120px");
	});
	...
	it("color", () => {
      ...
      `
		...
		expect(
			vm.$el.querySelector(".el-progress-bar__inner").style
				.backgroundColor
		).to.equal("rgb(0, 0, 0)");
	});
